...
def main(argv: Optional[list] = None) -> int:
    """Transform raw NWS observation payloads into daily station summaries."""
    ...
    try:
        ...
        invalid_condition = (
            col("stationId").isNull()
            | col("date").isNull()
            | (col("temperature_c").isNull() & col("wind_speed_mps").isNull())
        )
        ...
        df_daily = (
            df_valid
            .groupBy("stationId", "run_date", "date")
            .agg(
                first("stationName", ignorenulls=True).alias("stationName"),
                first("geometry_coordinates", ignorenulls=True).alias("geometry_coordinates"),
        ...
        logger.debug("Saved curated NWS parquet to: %s", out_dir)
        return 0
    finally:
        spark.stop()
